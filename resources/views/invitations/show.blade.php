...
<x-app-layout>
    <x-common.card>
        ...
        <div class="mb-4 border border-gray-100 border-gray-200 p-2">{!! Str::markdown($invitation->message) !!}</div>
        <x-slot name="footer" class="flex justify-between">
            <a
                class="'inline-flex duration-150' items-center rounded-md border border-sky-300 bg-white px-4 py-2 text-xs font-semibold tracking-widest text-sky-700 uppercase shadow-sm transition ease-in-out hover:bg-sky-50 focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:outline-none disabled:opacity-25 dark:border-sky-500 dark:bg-sky-800 dark:text-sky-300 dark:hover:bg-sky-700 dark:focus:ring-offset-sky-800"
