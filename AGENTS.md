...
### Public pages are stateless for cookieless guests (edge-cached by Cloudflare)

The public route group in `routes/web.php` (`/`, `/acts`, act pages, `/terms`, `/policy`, `/about`) drops the web group's `StartSession`, `ShareErrorsFromSession` and `PreventRequestForgery` and uses `SkipSessionForCookielessGuests` + `SetPageCacheHeaders` instead. A GET from a visitor with no session or `remember_*` cookie gets no session, no cookies and no CSRF token, and is sent as `public, max-age=0, s-maxage=300` so Cloudflare can cache one copy for every guest; everyone else gets the normal session and a `private` response. Consequences when editing these pages: anything rendered **for guests** must not need a session — no forms, no Livewire components, no `session()`/flash data, no `Cookie::queue()` (a cookie on the response makes it private and uncacheable). Member-only UI inside `@auth` is fine. Test guest behaviour with a plain `$this->get()` and member/session behaviour with `actingAs()` or `withCookie(config('session.cookie'), …)`.
